...
class App extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    // Comparo el estado previo de los filtros y si cambia, se modifica el estado con los filtros llamando a la funcion filtersHotels()
    if (this.state.filters !== prevState.filters) {
      this.setState({ filteredHotels: this.filterHotels() });
    }
  }

  // Cuando se selecciona una opcion nueva en el input, se modifica en el estado el nuevo filtro aplicado
  ...
  filterHotels = () => {
    ...
    if (hotelsFilteredByRooms && hotelsFilteredByRooms.length > 0) {
